...
--
-- IMPORTANTE: Antes de cargar este script en el servidor debes importar
-- los archivos SQL incluidos en `Examples/ClancyChestSystem/sql/` en la
-- base de datos `world`. Consulta el README.md adjunto para los pasos.
...
--
-- Tablas (deben existir en la base de datos `world`; ver los archivos
-- SQL en `Examples/ClancyChestSystem/sql/`):
...
local function IsStockEmpty()
    for _, data in pairs(State.items) do
...
local function BuildSnapshotPayload()
    ...
    for _, item in ipairs(stock) do
        -- Formato: entry|kind|amount|chance|name
        table.insert(
            lines,
            tostring(item.entry)         .. "|" ..
...
Deactivate = function(player, reason)
    ...
    -- No borramos la tabla de loot actual aquí, para que el conteo histórico
    -- quede visible. Se limpia en activaciones futuras según retención.
    UsedChests = {}
